...
def tmdb_get_sync(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raise HTTPException(
        status_code=502,
        detail=f"TMDB request error after 10 attempts: {type(last_err).__name__} | {repr(last_err)}",
    )
...
async def search_bundle(
    ...
    async def get_card_item(title: str, score: float) -> TFIDFRecItem:
        card = await attach_tmdb_card_by_title(title)
        return TFIDFRecItem(title=title, score=score, tmdb=card)

    tasks = [get_card_item(title, score) for title, score in recs]
    tfidf_items = list(await asyncio.gather(*tasks))
